...
class JustB2B_Related_Products {
	...
	private function set_min_max_values() {
		...
		$this->min_value = min( $mins );
		$this->max_value = max( $maxs );
	}
	...
	public function generate_related_products_html( $product_id, $quantity, $selected_option = null ) {
		if ( ! $this->is_test_category_product( $product_id ) ) return '';
		...
		foreach ( $this->related_products as $related ) {
			if ( ! in_array( $related['id'], $valid_ids, true ) ) continue;
	...
	public function enqueue_scripts() {
		global $product;
		if ( ! $product || ! $this->is_test_category_product( $product->get_id() ) ) return;
	...
	public function add_related_product_to_cart( $cart_item_data, $product_id ) {
		if ( ! $this->is_test_category_product( $product_id ) ) return $cart_item_data;
		...
		$cart_item_data['extra_product_name'] = $related->get_name();
		$cart_item_data['extra_product_price'] = $related->get_price();
	...
	public function update_cart_item_price( $cart ) {
		foreach ( $cart->get_cart() as $key => $item ) {
			...
			if ( isset( $item['extra_product_price'] ) ) {
				$unit_addon = $item['extra_product_price'] / max(1, $item['quantity']);
				$price = $item['data']->get_price();
				$item['data']->set_price( $price + $unit_addon );
			}
		}
	}
}
